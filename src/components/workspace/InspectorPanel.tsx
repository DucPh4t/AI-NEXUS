      ...
      <div className="h-12 px-4 border-b border-white/10 flex items-center justify-between bg-[#12141a]">
        {!isCollapsed && (
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <span className="text-[10px] font-mono bg-white/5 border border-white/10 px-1.5 py-0.5 rounded text-white/80 font-bold">
              {room.sourceHandle}
            </span>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-[10px] font-mono px-2 py-0.5 rounded-full font-bold ${
                  isNeedsReview
                    ? 'bg-amber-500/15 text-[#ffc474] border border-amber-500/30'
                    : isConfirmed
          ...
          <div className="grid grid-cols-2 gap-2.5 font-mono">
            ...
            <div className="p-3 rounded-xl bg-[#14161f] border border-white/10">
              ...
              <div className="text-xl font-bold text-white mt-0.5">
                {room.netPaintArea.toFixed(2)} <span className="text-xs font-normal text-white/40">m²</span>
              </div>
          ...
          <div className="p-3.5 rounded-xl bg-[#14161f] border border-white/10 font-mono text-[11px] space-y-1.5">
            ...
            <div className="flex justify-between text-white/70">
              ...
              <span className="text-white/80 font-mono">-{room.doorDeductions.toFixed(2)} m²</span>
            </div>
            <div className="flex justify-between text-white/70">
              <span>Khấu trừ cửa sổ:</span>
              <span className="text-white/80 font-mono">-{room.windowDeductions.toFixed(2)} m²</span>
            </div>
            <div className="pt-1 border-t border-white/10 flex justify-between text-white font-bold">
              <span>Tổng diện tích sơn tường:</span>
              <span className="text-white font-bold">{room.netPaintArea.toFixed(2)} m²</span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl border border-white/10 bg-[#14161f] space-y-1.5">
            ...
            <div className="flex items-center justify-between text-xs font-mono font-bold text-white pt-1.5 border-t border-white/10">
              ...
              <span className="text-white font-mono font-bold text-sm">
                {room.totalCost.toLocaleString()} <span className="text-xs text-white/50 font-normal">₫</span>
              </span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-[#0c0e12] text-white font-mono text-xs space-y-2 border border-white/10">
            <div className="flex items-center justify-between text-white/40 text-[10px] uppercase font-bold">
              ...
              <span className="text-white/70 font-semibold">● {room.confidence}% TIN CẬY</span>
            </div>

            <div className="space-y-1 text-[11px] text-white/70">
              <div className="flex justify-between">
                <span className="text-white/40">Handle DXF:</span>
                <span className="text-white font-mono font-semibold">{room.sourceHandle}</span>
              </div>
              <div className="flex justify-between">
        ...
        <div className="p-4 border-t border-white/10 bg-[#12141a] space-y-2 shrink-0">
          <div className="grid grid-cols-3 gap-2 text-xs font-medium">
            <button
              ...
              className="amber-button py-2.5 text-xs font-semibold text-center transition-all shadow-xs flex items-center justify-center gap-1.5 cursor-pointer"
            >
